						...
						<Table size='small'>
							...
							<TableHead>
								<TableRow>
									...
									<TableCell align='center' sx={{fontWeight: 'bold'}}>Tổng số tiết/tuần</TableCell>
								</TableRow>
							</TableHead>

							{/* Body */}
							<TableBody>
								{dataStored && dataStored['teacher-assignments-summary'].map((teacher: ITeacherAssignmentSummary) => (
									<React.Fragment key={teacher['teacher-id']}>
										{/* Hàng chính (giáo viên) */}
										<TableRow sx={{bgcolor: '#f5f5f5', cursor: 'pointer'}}  onClick={() => toggleRow(teacher['teacher-id'])}>
											<TableCell width={50}>
													...
													<Table size='small'>
														<TableHead>
															<TableRow>
																...
																<TableCell sx={{fontWeight: 'bold'}}>Viết Tắt</TableCell>
																<TableCell align='center' sx={{fontWeight: 'bold'}}>Số tiết/tuần</TableCell>
															</TableRow>
														</TableHead>
														<TableBody>
															{teacher['total-periods-per-week'].map((subject: ITeacherPeriodsPerWeek) => (
																<TableRow key={subject['subject-id']}>
                                                                    <TableCell width={50} />
																	<TableCell width={300}>{subject['subject-name']}</TableCell>
																	<TableCell width={200}>{subject['subject-abbreviation']}</TableCell>
