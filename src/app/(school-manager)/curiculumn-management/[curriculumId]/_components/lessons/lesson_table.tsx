		...
		if (typeof value === 'string') {
			if (Number(value) < 0) {
				...
				setSumObject((prev: ISumObject) => ({
					...prev,
					[target]: ((prev as any)[target] ?? 0) + Number(value as string) - previousValue,
				}));
			}
						...
						<TableBody>
							...
							{subjectTableData.map((row, index) => {
								...
								const editedObject: IUpdateSubjectInGroupRequest | undefined =
									editingObjects.find((item) => item['curriculum-detail-id'] === row.id) ??
									undefined;
								...
								return (
									<TableRow
										...
										sx={[
											editedObject !== undefined && {
												bgcolor: vulnarableIndexes.includes(editedObject['curriculum-detail-id'])
													? 'rgba(245, 75, 75, .2)'
													: '#edf1f5',
											},
										]}
									>
										...
										>
											<Tooltip
												title={row.isRequiredSubject ? 'Môn học bắt buộc' : 'Môn học tự chọn'}
											>
												<Typography fontSize={15}>{row.lessonName}</Typography>
											</Tooltip>
										</TableCell>
											...
											{isEditingMode ? (
												<TextField
													...
													onChange={(event: ChangeEvent<HTMLInputElement>) => {
														...
														handleUpdateLesson('main-slot-per-week', value, row);
													}}
													value={
														!editedObject
												...
												<Typography fontSize={15}>{row.mainTotalSlotPerWeek}</Typography>
											)}
										</TableCell>
										<TableCell align='center' width={100}>
											{isEditingMode ? (
												<TextField
													...
													onChange={(event: ChangeEvent<HTMLInputElement>) => {
														...
														event.target.value = value;
														handleUpdateLesson('main-minimum-couple', value, row);
													}}
													value={
														!editedObject
												...
												<Typography fontSize={15}>{row.mainMinimumCouple}</Typography>
											)}
										</TableCell>
											...
											{isEditingMode ? (
												<TextField
													...
													onChange={(event: ChangeEvent<HTMLInputElement>) => {
														...
														event.target.value = value;
														handleUpdateLesson('sub-slot-per-week', value, row);
													}}
													value={
														!editedObject
												...
												<Typography fontSize={15}>{row.subTotalSlotPerWeek}</Typography>
											)}
										</TableCell>
										<TableCell align='center' width={100}>
											{isEditingMode ? (
												<TextField
													...
													onChange={(event: ChangeEvent<HTMLInputElement>) => {
														...
														event.target.value = value;
														handleUpdateLesson('sub-minimum-couple', value, row);
													}}
													value={
														!editedObject
												...
												<Typography fontSize={15}>{row.subMinimumCouple}</Typography>
											)}
										</TableCell>
										...
										<TableCell align='center' width={50}>
											<Checkbox
												color='default'
												disabled
												checked={!editedObject ? row.isDouleSlot : editedObject['is-double-period']}
											/>
										</TableCell>
