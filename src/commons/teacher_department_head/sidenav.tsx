...
import {
  TEACHER_HEAD_SIDENAV,
  ITeacherHeadNavigation,
} from "@/app/(teacher-department-head)/_utils/constants";
import { Collapse } from "@mui/material";
import Image from "next/image";
import Link from "next/link";
import { usePathname, useRouter } from "next/navigation";
import "../styles/teacher_head_sidenav.css";
import { useAppContext } from "@/context/app_provider";
import {
  ITeacherHeadInfo,
  ITeacherHeadState,
  setTeacherHeadInfo,
} from "@/context/slice_teacher_head";
import {
  useTeacherHeadDispatch,
  useTeacherHeadSelector,
} from "@/hooks/useReduxStore";

const TeacherHeadSidenav = () => {
  const currentPath = usePathname();
  const router = useRouter();
  const dispatch = useTeacherHeadDispatch();
  const { isMenuOpen }: ITeacherHeadState = useTeacherHeadSelector(
  ...
  const handleLogout = async () => {
    dispatch(setTeacherHeadInfo({} as ITeacherHeadInfo));
    await logout();
  };
      ...
      <div className="relative w-[16vw] h-full flex flex-col justify-start items-start gap-5 bg-white border-r-1 border-gray-400">
        ...
        <div className="w-full h-fit pb-[70px] pt-[50px] flex flex-col justify-start items-center overflow-y-scroll no-scrollbar">
          <div className="w-full h-full p-3">
            {TEACHER_HEAD_SIDENAV.map(
              (item: ITeacherHeadNavigation, index: number) => (
                <div
                  key={item.name}
...
export default TeacherHeadSidenav;
