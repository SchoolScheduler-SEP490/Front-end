...
const TeacherHeadHeader = ({ children }: { children: ReactNode }) => {
  const {
  ...
  useEffect(() => {
    ...
    useEffect(() => {
      if (!teacherHeadInfo && sessionToken) {
        ...
        .then(data => {
          if (data.status === 200) {
            dispatch(setTeacherHeadInfo(data.result));
          }
        });
      }
    }, [sessionToken, schoolId, teacherHeadInfo]);
          ...
          <div className="flex flex-row justify-end items-center gap-3">
            ...
            <div className="w-fit h-full flex flex-col justify-between items-end text-white pr-3">
              <div className="text-body-medium font-medium leading-4 pr-1">
                {teacherHeadInfo
                  ? `${teacherHeadInfo["first-name"]} ${teacherHeadInfo["last-name"]}`
                  : userRole}
              </div>
              <div
...
export default TeacherHeadHeader;
